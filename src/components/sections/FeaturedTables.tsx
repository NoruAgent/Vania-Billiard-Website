    ...
    <section className="py-20 lg:py-32 bg-bg">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {featured.map((p, i) => (
            <Reveal key={p.id} delay={i * 120} y={28}>
        ...
        {prime && (
          <Reveal delay={200}>
            <div className="mt-12 lg:mt-16 grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-8 bg-surface border border-border-subtle overflow-hidden group hover-lift">
              <div className="relative aspect-[4/3] lg:aspect-auto overflow-hidden">
                <img
                  src={`/images/products/${prime.slug}.jpg`}
                  alt={prime.name}
                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 img-reveal"
                  loading="lazy"
                />
